 ...
 */
export function createToolbox(blocks) {
 ...
 */
export function blockOfType(type, titles) {
  var titleText = '';
  if (titles) {
 ...
 */
export function blockWithNext(type, titles, child) {
  var titleText = '';
  if (titles) {
    for (var key in titles) {
      titleText += '<title name="' + key + '">' + titles[key] + '</title>';
 ...
 * the previous block.
 */
export function blocksFromList(types) {
  if (types.length === 1) {
 ...
 */
export function createCategory(name, blocks, custom) {
 ...
 */
export function generateSimpleBlock(blockly, generator, options) {
 ...
 * @returns {*}
 */
export function domToBlock(blockDOM) {
 ...
 * @returns {*}
 */
export function domStringToBlock(blockDOMString) {
 ...
 */
export function forceInsertTopBlock(input, blockType) {
  input = input || '';
 ...
 */
export function calcBlockXml(type, args) {
 ...
 *   variableName
 */
export function calcBlockGetVar(variableName) {
  return '' +
 ...
 * @param {Object.<string.string>} [titles] Dictionary of titles mapping name to value
 */
export function mathBlockXml(type, inputs, titles) {
  var str = '<block type="' + type + '" inline="false">';
 ...
 */
export function functionalDefinitionXml(name, outputType, argList, blockXml) {
 ...
 * @param {Object<string, string>[]} argList Name and type for each arg
 */
export function functionalCallXml(name, argList, inputContents) {

...
// import api from './api';
import apiBlockly from './apiBlockly';
...
// var MEDIA_PROXY = '//' + window.location.host + '/media?u=';
...
var designMode = DesignMode(Applab);
...
function adjustMediaHeightRule(mediaList, defaultHeightRules, newHeightRules) {
  ...
  if (mediaList.length === 2) {
    ...
    for (var i = 0; i <= lastHeightRuleIndex; i++) {
      if (mediaList.item(1).indexOf('(min-height: ' +
          (defaultHeightRules[i] + 1) + 'px)') !== -1) {
        if (i === 0) {
...
function hasSeenDataAlert(channelId) {
  var dataAlerts = window.localStorage.getItem('dataAlerts');
  if (!dataAlerts) {
    return false;
  }
...
function markSeenDataAlert(channelId) {
  ...
  window.localStorage.setItem('dataAlerts', JSON.stringify(channelIds));
}
...
Applab.startSharedAppAfterWarnings = function () {
  ...
  var is13Plus = Applab.user.isSignedIn || window.localStorage.getItem('is13Plus') === 'true';
  var _handleClose = onCloseShareWarnings.bind(null, showStoreDataAlert);
...
function showInstructionsInTopPane() {
  ...
  if (/topInstructions=true/.test(window.location.search)) {
    window.localStorage.setItem('showInstructionsInTopPane', true);
  }
  // disable instructions in top pane based on query param
  if (/topInstructions=false/.test(window.location.search)) {
    window.localStorage.removeItem('showInstructionsInTopPane');
  }
  return !!window.localStorage.getItem('showInstructionsInTopPane');
}

import controls from './controls.html.ejs';
import visualization from './visualization.html.ejs';

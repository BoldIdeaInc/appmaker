 ...
 */
export function evalWith(code, options) {
  if (options.StudioApp && options.StudioApp.editCode) {
    // Use JS interpreter on editCode levels
    var initFunc = function (interpreter, scope) {
      initJSInterpreter(interpreter, null, null, scope, options);
    };
 ...
 */
export function functionFromCode(code, options) {
  if (options.StudioApp && options.StudioApp.editCode) {
...
export function loopHighlight(apiName, blockId) {
 ...
 * Extract the user's code as raw JavaScript.
 */
export function workspaceCode(blockly) {
  var code = blockly.Javascript.blockToCode();
  return strip(code);
};
...
function marshalNativeToInterpreterObject(
  ...
  for (var prop in nativeObject) {
    ...
    interpreter.setProperty(retVal,
                            prop,
                            marshalNativeToInterpreter(interpreter,
                                                               value,
                                                               nativeObject,
                                                               maxDepth));
  }
...
export const customMarshalObjectList = [];
export const asyncFunctionList = [];
export const nativeCallsInterpreterFunctionList = [];
...
var shouldCustomMarshalObject = function (nativeVar, nativeParentObj) {
  for (var i = 0; i < customMarshalObjectList.length; i++) {
    var marshalObj = customMarshalObjectList[i];
...
export function marshalNativeToInterpreter(interpreter, nativeVar, nativeParentObj, maxDepth) {
  ...
  if (nativeVar instanceof Array) {
    ...
    if (asyncFunctionList.indexOf(nativeVar) !== -1) {
      // Mark if this should be nativeIsAsync:
      makeNativeOpts.nativeIsAsync = true;
    }
    if (nativeCallsInterpreterFunctionList.indexOf(nativeVar) !== -1) {
...
export const createNativeFunctionFromInterpreterFunction = null;

export function marshalInterpreterToNative(interpreter, interpreterVar) {
  if (interpreterVar.isPrimitive || interpreterVar.isCustomMarshal) {
    ...
    for (var i = 0; i < nativeArray.length; i++) {
      nativeArray[i] = marshalInterpreterToNative(interpreter,
    ...
    for (var prop in interpreterVar.properties) {
      nativeObject[prop] = marshalInterpreterToNative(interpreter,
                                                              interpreterVar.properties[prop]);
    }
    return nativeObject;
  } else if (interpreter.isa(interpreterVar, interpreter.FUNCTION)) {
    if (createNativeFunctionFromInterpreterFunction) {
      return createNativeFunctionFromInterpreterFunction(interpreterVar);
    } else {
 ...
 */
export function createNativeInterpreterCallback(opts, intFunc) {
  return function (nativeValue) {
...
export function makeNativeMemberFunction(opts) {
  if (opts.dontMarshal) {
    return function () {
      ...
      return marshalNativeToInterpreter(opts.interpreter, nativeRetVal,
        null, opts.maxDepth);
    };
  } else {
    return function () {
      ...
      for (var i = 0; i < arguments.length; i++) {
        if (opts.nativeIsAsync && (i === arguments.length - 1)) {
          ...
          nativeArgs[i] = marshalInterpreterToNative(opts.interpreter, arguments[i]);
        }
      }
...
function populateJSFunctions(interpreter) {
  ...
  for (var i = 0; i < functions.length; i++) {
    wrapper = makeNativeMemberFunction({
      interpreter: interpreter,
 ...
 */
export function initJSInterpreter(interpreter, blocks, blockFilter, scope, globalObjects) {
 ...
 */
export function isNextStepSafeWhileUnwinding(interpreter) {
...
export function calculateCumulativeLength(code) {
  var regex = /\n/g, result = [];
  do {
...
export function aceFindRow(cumulativeLength, rows, rowe, pos) {
  if (rows > rowe) {
    return null;
  }
  if (rows + 1 === rowe) {
...
export function isAceBreakpointRow(session, userCodeRow) {
  if (!session) {
    return false;
  }
 ...
 */
export function selectEditorRowColError(editor, row, col) {
  if (!editor) {
    return;
  }
 ...
 */
export function clearDropletAceHighlighting(editor, allClasses) {
  if (editor.currentlyUsingBlocks) {
    editor.clearLineMarks();
  } else {
 ...
 */
export function selectCurrentCode(interpreter,
                                      cumulativeLength,
                                      userCodeStartOffset,
                                      userCodeLength,
                                      editor,
  ...
  if (interpreter && interpreter.stateStack[0]) {
    ...
    if (node.type === 'ForStatement') {
      ...
      switch (mode) {
        case ForStatementMode.INIT:
          subNode = node.init;
          break;
        case ForStatementMode.TEST:
          subNode = node.test;
          break;
        case ForStatementMode.BODY:
          subNode = node.body;
          break;
        case ForStatementMode.UPDATE:
          subNode = node.update;
          break;
      }
      node = subNode || node;
    ...
    if (start >= 0 && start < userCodeLength && end <= userCodeLength) {
      ...
      clearDropletAceHighlighting(editor);
    }
  } else {
    clearDropletAceHighlighting(editor);
  }
